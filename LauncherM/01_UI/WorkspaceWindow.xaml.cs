...
namespace LauncherM
{
    public partial class WorkspaceWindow : Window
    {
        ...
        private void WorkspaceChanged(object sender, SelectionChangedEventArgs e)
        {
            ...
            foreach (LaunchItem item in workspace.LaunchItems) { Button tile = new Button { Content = item.Name, Tag = item, Style = (Style)FindResource("TileStyle") }; tile.Click += SelectItem; itemsPanel.Children.Add(tile); }
            ClearDetails();
        }
        private void SelectItem(object sender, RoutedEventArgs e) { selectedItem = (LaunchItem)((Button)sender).Tag; nameText.Text = selectedItem.Name; typeText.Text = selectedItem.Type.ToString(); targetText.Text = selectedItem.Target; argumentsText.Text = selectedItem.Arguments; workingDirectoryText.Text = selectedItem.WorkingDirectory; }
        ...
        private void ClearDetails() { selectedItem = null; nameText.Text = typeText.Text = targetText.Text = argumentsText.Text = workingDirectoryText.Text = ""; }
    }
}
