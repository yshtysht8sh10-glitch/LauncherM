...
class LaunchItemNameResolverChecks
{
    ...
    static async Task Run()
    {
        ...
        Check(resolver.Resolve(Item(LaunchItemType.Url, "https://chatgpt.com/example")) == "ChatGPT", "Web hostname fallback");
        ...
        int port = 41000 + Math.Abs(Guid.NewGuid().GetHashCode() % 10000);
        ...
        using (var listener = new HttpListener())
        {
            ...
            Task server = Task.Run(async () =>
            {
                ...
                byte[] body = Encoding.UTF8.GetBytes("<html><head><title>ChatGPT &amp; LauncherM</title></head></html>");
                context.Response.ContentType = "text/html; charset=utf-8";
                context.Response.OutputStream.Write(body, 0, body.Length); context.Response.Close();
            });
            Check(await resolver.ResolveWebTitleAsync(prefix, CancellationToken.None) == "ChatGPT & LauncherM", "HTML title and entity decode");
            await server;
        }
        Check(await resolver.ResolveWebTitleAsync("not-a-url", CancellationToken.None) == null, "Invalid URL is silent");
        Console.WriteLine("LaunchItemNameResolverChecks: 12 checks passed.");
    }
