...
タブとカードのドラッグで表示した挿入用の隙間は、親コンテナ側でも同じ挿入位置としてドロップを受け付けます。
...
設定ウィンドウはWindowsのインストール済みフォント、全体文字サイズ倍率、テーマカラーを選択し、ユーザー設定へ保存します。設定内容だけをスクロール領域に置き、コントロールと縦スクロールバーの間に余白を確保し、適用・OK・キャンセルはウィンドウ下部へ固定表示します。ダーク背景で使うCheckBoxは明るい文字色を明示します。Workspace画面は設定変更イベントを受けてフォント・倍率・背景・前景・アクセントを再適用します。
